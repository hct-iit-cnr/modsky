...
import { getInterventionConfig, type InterventionConfig, type InterventionPosition, TEST_INTERVENTIONS } from '#/lib/interventionEngine';
...
export const ComposePost = ({
  ...
  // --- TELEMETRY TRACKER REFS ---
  const telemetryLog = useRef<{ event: string; timestamp: number; payload?: any; }[]>([]);
  const mountTimeRef = useRef(Date.now());
  const firstKeystrokeTimeRef = useRef<number | null>(null);
  const currentPhaseRef = useRef<'none' | 'loading' | 'reading' | 'revealed'>('none');
  ...
  const previousTextRef = useRef(thread.posts[composerState.activePostIndex]?.richtext.text || '');
  ...
  const onClose = useCallback(() => {
    // --- NEW METRIC: INTERVENTION_ABORTED ---
    if (currentPhaseRef.current === 'loading' || currentPhaseRef.current === 'reading') {
      logTelemetry('INTERVENTION_ABORTED', {
        phase: currentPhaseRef.current,
        timeSpentMs: Date.now() - phaseStartTimeRef.current
      });
    }
    ...
    revokeAllMediaUrls()
  }, [closeComposer, queryClient, logTelemetry])
  ...
  const onPressPublish = React.useCallback(async () => {
    ...
    // --- NEW METRIC: TIME_TO_PUBLISH ---
    logTelemetry('TIME_TO_PUBLISH', {
      durationMs: Date.now() - mountTimeRef.current,
      timeSinceFirstKeystrokeMs: firstKeystrokeTimeRef.current ? Date.now() - firstKeystrokeTimeRef.current : 0
    });
...
// --- MODSKY INTERVENTION LOGIC START ---
  ...
  // --- NEW METRIC: APP BACKGROUND LISTENER ---
  const appState = useAppState();
  useEffect(() => {
    if (appState === 'background' || appState === 'inactive') {
      logTelemetry('COMPOSER_BACKGROUNDED', {
        textLength: activePost?.richtext.text.length || 0,
        phase: currentPhaseRef.current
      });
    }
  }, [appState, activePost?.richtext.text.length, logTelemetry]);
  ...
  useEffect(() => {
    ...
    // FOR ALL INTERVENTIONS: Phase 1 (Loading) -> Phase 2 (Reading) -> Phase 3 (Composer Reveals)
    let readingTimer: NodeJS.Timeout;
    
    const loadingTimer = setTimeout(() => {
      setIsFakeLoading(false); // Phase 1 ends: Reveal the intervention text
      logTelemetry('INTERVENTION_REVEALED');
      currentPhaseRef.current = 'reading';
      phaseStartTimeRef.current = Date.now();
      
      const readingDelayMs = interventionState.readingDuration || 5000; 
      
      readingTimer = setTimeout(() => {
        setIsComposerRevealed(true); // Phase 2 ends: Reveal the composer box
        currentPhaseRef.current = 'revealed';
        phaseStartTimeRef.current = Date.now();
  ...
  const renderIntervention = (targetPosition: InterventionPosition) => {
    if (!interventionState || !interventionState.isVisible) return null;

    const effectivePosition = interventionState.position === 'middle' && !replyTo 
      ? 'top' 
      : interventionState.position;

    if (effectivePosition === 'placeholder' || effectivePosition !== targetPosition) return null;

    // We hide the intervention banner entirely during fake loading, regardless of loading type
    const isHiddenPhase = isFakeLoading;
    const isTitle = targetPosition === 'title';

    const getIconComponent = () => {
      switch (interventionState.iconType) {
        case 'info': return CircleInfoIcon;
        case 'smile': return EmojiSmileIcon;
    ...
    const getIconColor = () => {
      if (isTitle) return '#FFFFFF'; // Force white icons against the solid blue background
      
      if (interventionState.iconType === 'info' || interventionState.iconType === 'smile') {
        return t.palette.primary_500; // Blue from theme
      }
      if (interventionState.iconType === 'warning') {
        return '#FFD700'; // Gold
      }
      if (interventionState.iconType === 'leaf') {
        return t.atoms.text.color; // black / white (theme text color)
      }
      return t.palette.primary_500;
    };

    const getBgColor = () => {
      // If it's a title, the background is handled by the outer container to perfectly match the native corners!
      if (isTitle) return 'transparent';
      
      if (interventionState.iconType === 'leaf') {
        // Light green / Dark green dynamic bg based on theme
        return t.name === 'light' ? '#E8F5E9' : '#1B5E20';
      }
      return t.atoms.bg_contrast_25.backgroundColor;
    };
    ...
    return (
      <Animated.View 
        ...
        style={[
          a.w_full, 
          isTitle 
            ? [a.py_md, a.px_lg] // Title has NO manual corners. The parent bottom-sheet clips it natively!
            : [a.px_lg, a.py_md, { borderRadius: 12, marginHorizontal: 16, width: 'auto', marginBottom: 16 }], // Floating box styling
          isHiddenPhase ? { backgroundColor: 'transparent' } : { backgroundColor: getBgColor() }
        ]}>
        <View style={[
          a.flex_row, a.align_center, 
          isTitle ? a.justify_center : a.justify_start, 
          a.gap_sm,
          isHiddenPhase ? { opacity: 0 } : {}
        ]}>
          {interventionState.hasIcon && IconComponent && (
            <IconComponent 
              size="md" 
              fill={getIconColor()} 
          ...
          <Text style={[
            ...
            a.text_md, 
            a.font_bold, 
            isTitle ? { color: '#FFFFFF', textAlign: 'center' } : t.atoms.text, // Force white text against the solid blue background
          ]}>
  ...
  const isTitleInterventionActive = !!(interventionState && interventionState.position === 'title' && interventionState.isVisible);
  ...
  // 2. Text Box Fade
  const isSkeletonPhaseGlobal = !isComposerRevealed && interventionState?.loadingType === 'placeholder';
  
  // Keep the text box visible if it's the skeleton phase so we can see the skeleton!
  const showTextBox = isComposerRevealed || isSkeletonPhaseGlobal || (
    interventionState?.position === 'placeholder' && 
    interventionState?.loadingType !== 'placeholder'
  );
  ...
  useEffect(() => {
    if (!currentText && !previousTextRef.current) return;

    if (currentText && !hasLoggedFirstKeystroke) {
    ...
    // --- NEW METRIC: TEXT_DELETION_BURST ---
    const textDelta = previousTextRef.current.length - currentText.length;
    if (textDelta > 10) {
      logTelemetry('TEXT_DELETION_BURST', {
        deletedLength: textDelta,
        timeSinceInterventionMs: phaseStartTimeRef.current ? Date.now() - phaseStartTimeRef.current : 0
      });
    }
    ...
    const timeoutId = setTimeout(() => {
      if (currentText) {
        logTelemetry('TEXT_INTERMEDIATE_SNAPSHOT', { 
          ...
          isAfterIntervention: !!interventionState && currentPhaseRef.current === 'revealed'
        });
      }
    }, 1000);
  ...
  // Safely extract the display name of the user you are replying to (if it exists)
  // Fall back to their handle if they haven't set a display name
  const replyAuthor = (replyTo as any)?.author;
  const replyToName = replyAuthor?.displayName || replyAuthor?.handle as string | undefined;

  // --- MODSKY: DEBUG MENU INTERCEPTION ---
  if (!interventionState) {
    return (
      <View style={[a.flex_1, t.atoms.bg, viewStyles, a.px_xl, a.pt_2xl]}>
        <View style={[a.flex_row, a.justify_between, a.align_center, a.mb_xl]}>
          <Text style={[a.text_2xl, a.font_bold, t.atoms.text]}>Select Intervention</Text>
          <Button
            label={_(msg`Cancel`)}
            variant="ghost"
            color="primary"
        ...
        <ScrollView contentContainerStyle={[a.gap_md, a.pb_2xl]}>
          {/* Explicit Control Group Button */}
          <Button
            label="Control (No Intervention)"
            variant="outline"
            color="primary"
            onPress={() => {
              const config = getInterventionConfig(!!replyTo, replyToName, -1);
              setInterventionState(config);
              setIsFakeLoading(config.isVisible);
              setIsComposerRevealed(!config.isVisible);
            }}
            style={[a.p_md, a.rounded_sm, a.align_start]}>
            <ButtonText style={[a.text_md]}>ID -1: Control (No Intervention)</ButtonText>
          </Button>
          
          {/* Loop through actual interventions based on Context */}
          {TEST_INTERVENTIONS.filter(ti => replyTo ? ti.isReply : ti.isNonReply).map(ti => (
            <Button
              key={ti.id}
              label={ti.label}
              variant="outline"
              color="primary"
              onPress={() => {
                // Pass the reply context and the display name dynamically
                const config = getInterventionConfig(!!replyTo, replyToName, ti.id);
                setInterventionState(config);
                setIsFakeLoading(config.isVisible);
                setIsComposerRevealed(!config.isVisible);
              }}
  ...
  // Extract Composer Body to easily conditionally wrap it
  const composerContent = (
    <>
      ...
      <Animated.ScrollView
        ...
        {/* POSITION 0: TITLE HAS BEEN MOVED ABOVE TOP BAR */}
        ...
        <View style={a.relative}>
          ...
          {/* MODSKY: FULL INTERFACE LOADER (SPINNER/DOTS) */}
          {/* This sits absolutely positioned ON TOP of the hidden interface elements until they are fully revealed */}
          {/* NOTE: It spins continuously during BOTH Phase 1 (loading box) and Phase 2 (reading box) */}
          {!isComposerRevealed && (interventionState?.loadingType === 'circle' || interventionState?.loadingType === 'lines') && interventionState?.position !== 'placeholder' && (
            <Animated.View 
              exiting={FadeOut.duration(300)} 
              style={[a.absolute, a.w_full, a.h_full, a.align_center, a.justify_center, { zIndex: 10 }]} 
              pointerEvents="none"
            >
              {interventionState.loadingType === 'circle' && <ActivityIndicator size="large" color={t.palette.primary_500} style={{ transform: [{ scale: 1.5 }] }} />}
              {interventionState.loadingType === 'lines' && <AnimatedDots />}
            </Animated.View>
          )}
          ...
          {replyTo ? (
            <View style={a.relative}>
              ...
              {/* REAL COMPONENT (Invisible but dictates exact height) */}
              <Animated.View style={generalFadeStyle} pointerEvents={isComposerRevealed ? 'auto' : 'none'}>
                <ComposerReplyTo replyTo={replyTo} />
              </Animated.View>

              {/* SKELETON OVERLAY (Perfectly maps to real component's bounding box) */}
              {isSkeletonPhaseGlobal && (
                <AnimatedSkeletonPulse 
                  style={[a.absolute, a.w_full, a.h_full, { top: 0, left: 0 }]}
                  exiting={FadeOut.duration(500)}
                >
                  <View style={[a.flex_row, a.px_lg, a.pt_md, a.pb_sm, a.h_full]}>
                    
                    {/* Avatar & Thread Line Skeleton */}
                    <View style={[a.align_center, { width: 42 }]}>
                      <View style={[a.rounded_full, { width: 42, height: 42, backgroundColor: t.palette.primary_100 }]} />
                      <View style={[{ width: 2, flex: 1, backgroundColor: t.palette.primary_100, borderRadius: 1, marginTop: 8 }]} />
                    </View>
                    
                    {/* Exact Match Text Block - Automatically stretches to fill the exact text height! */}
                    <View style={[a.flex_1, { paddingLeft: 10, paddingTop: 4, paddingBottom: 4 }]}>
                      <View style={[a.rounded_sm, a.w_full, a.h_full, { backgroundColor: t.palette.primary_100 }]} />
                    </View>
            ...
            <React.Fragment key={post.id + (composerState.draftId ?? '')}>
              <Animated.View style={textBoxFadeStyle} pointerEvents={isComposerRevealed ? 'auto' : 'none'}>
                <ComposerPost
                  ...
                  isSkeletonPhase={isSkeletonPhaseGlobal}
                />
              </Animated.View>
  ...
  return (
    <BottomSheetPortalProvider>
      <KeyboardAvoidingView
        ...
        {/* THIS CONTAINER MANAGES SAFE AREAS AND INHERITS NATIVE CORNERS */}
        <View
          style={[
            a.flex_1, 
            viewStyles,
            // Only if a title is active, we color this entire outer wrapper blue.
            // The parent BottomSheet natively clips the top rounded corners flawlessly!
            isTitleInterventionActive ? { backgroundColor: t.palette.primary_500 } : {}
          ]}
          aria-modal
          accessibilityViewIsModal>
          
          {/* ---> NEW LOCATION: POSITION 0: TITLE <--- */}
          {/* Notice it has NO manual curves now! */}
          {renderIntervention('title')}

          {/* If the title is active, we wrap the rest of the composer in the standard white/black background. */}
          {/* It uses square corners, attaching perfectly flush to the bottom of the blue title above it. */}
          {isTitleInterventionActive ? (
            <View style={[a.flex_1, t.atoms.bg]}>
              {composerContent}
            </View>
          ) : (
            // If no title, render natively with no wrappers to avoid messing up any native styles
            composerContent
          )}
  ...
  return (
    <View
      ...
      <View style={[a.flex_row, IS_NATIVE && a.flex_1]}>
        ...
        {/* AVATAR + SKELETON */}
        <View style={a.relative}>
          <View style={isSkeletonPhase ? { opacity: 0 } : {}}>
            <UserAvatar
          ...
          {isSkeletonPhase && (
            <AnimatedSkeletonPulse style={[a.absolute, a.mt_xs, { width: 42, height: 42, borderRadius: 21, backgroundColor: t.palette.primary_100, top: 0, left: 0 }]} />
          )}
        </View>
        
        {/* TEXT INPUT + SKELETON */}
        <View style={[a.flex_1, a.relative, { paddingLeft: 10 }]} pointerEvents={isFakeLoading ? 'none' : 'auto'}>
          <View style={isSkeletonPhase ? { opacity: 0 } : {}}>
            <TextInput
              ref={textInput}
              editable={true} 
              caretHidden={isFakeLoading} 
              style={[a.pt_xs]}
              richtext={richtext}
          ...
          {isSkeletonPhase && (
            <AnimatedSkeletonPulse style={[a.absolute, a.pt_xs, a.w_full, { top: 0, left: 0 }]}>
              {/* Magic trick: perfectly fits the placeholder length */}
              <Text style={[
                a.text_lg, 
                { color: 'transparent', backgroundColor: t.palette.primary_100, borderRadius: 4, alignSelf: 'flex-start' }
              ]}>
                {selectTextInputPlaceholder}
              </Text>
            </AnimatedSkeletonPulse>
          )}
        </View>
...
function AnimatedDots() {
  return (
    <View style={[a.flex_row, a.gap_md, a.align_center, a.justify_center, { height: 48 }]}>
      <AnimatedDot delay={0} />
...
function AnimatedDot({ delay }: { delay: number }) {
  ...
  return (
    <Animated.View style={[{ width: 16, height: 16, borderRadius: 8, backgroundColor: t.palette.primary_500 }, style]} />
  );
}

function AnimatedSkeletonPulse({ children, style, exiting }: { children: React.ReactNode, style?: any, exiting?: any }) {
  ...
  return (
    <Animated.View style={[style, animatedStyle]} exiting={exiting}>
      {children}
    </Animated.View>
  );
}